...
def calc(choice):
    if choice == '+':
        ...
        try:
            return elevation(int(input('Введіть перше число: ')), int(input('Введіть друге число: ')))
        except ValueError:
            logging.error("ValueError", exc_info=True)
            return 'Введено не число. Спробуйте ще раз'
    elif choice == '%':
        try:
            return search_percentage(int(input('Введіть перше число: ')), int(input('Введіть друге число: ')))
        except ValueError:
            logging.error("ValueError", exc_info=True)
            return 'Введено не число. Спробуйте ще раз'
    elif choice == 'sqrt':
        try:
            return taking_sqrt(int(input('Введіть перше число: ')))
        except ValueError:
            logging.error("ValueError", exc_info=True)
            return 'Введено не число. Спробуйте ще раз'
    else:
...
def search_percentage(num_of_percent, num):
    ...
    return f'Відповідь: {num_of_percent}% from {num} = {num/100 * num_of_percent}'
